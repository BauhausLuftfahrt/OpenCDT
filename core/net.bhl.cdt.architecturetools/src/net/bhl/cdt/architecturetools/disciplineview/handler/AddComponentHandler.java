 ...
 ******************************************************************************/
package net.bhl.cdt.architecturetools.disciplineview.handler;
...
public class AddComponentHandler extends AbstractCDTHandler implements IHandler {
	...
	public Object doExecute(ExecutionEvent event) throws ExecutionException {
		...
		if (selection instanceof Component) {
			ECPProject ecpProject = ECPWorkspaceManager
					.getECPProject(selection);
			...
			// };
			dialog.setModelElements(UtilitiesHelper.getChildrenOfType(
					((ConfigurationInput) input).getConfiguration(),
					Component.class));
			if (dialog.open() == Dialog.OK) {
				...
				new CDTCommand() {
					...
					protected void doRun() {
						ViewLink viewLink = ModelFactory.eINSTANCE
								.createViewLink();
